...
__Board::__Board() : pos(DefaultStartPositionSFEN) {}
__Board::__Board(const std::string& sfen) : pos(sfen) {}
__Board::~__Board() {}

void __Board::set(const std::string& sfen) {
  history.clear();
  pos.set(sfen);
}

void __Board::set_position(const std::string& position) {
  history.clear();
...
void __Board::set_pieces(const int pieces[], const int pieces_in_hand[][7]) {
  history.clear();
  pos.set((const Piece*)pieces, pieces_in_hand);
}

void __Board::set_hcp(char* hcp) {
  history.clear();
  pos.set_hcp(hcp);
}

void __Board::set_psfen(char* psfen) {
  history.clear();
  pos.set_psfen(psfen);
}
...
std::string __Board::dump() const {
  std::stringstream ss;
  pos.print(ss);
  return ss.str();
}

void __Board::push(const int move) {
...
int __Board::get_last_move() const {
  if (history.size() > 0)
    return history.back().first.value();
  else
    return Move::moveNone().value();
...
bool __Board::is_game_over() const {
  const MoveList<LegalAll> ml(pos);
  return ml.size() == 0;
}

int __Board::isDraw(const int checkMaxPly) const { return (int)pos.isDraw(checkMaxPly); }
int __Board::moveIsDraw(const int move, const int checkMaxPly) const { return (int)pos.moveIsDraw(Move(move), checkMaxPly); }

int __Board::move(const int from_square, const int to_square, const bool promotion) const {
  if (promotion)
...
int __Board::drop_move(const int to_square, const int drop_piece_type) const {
  return makeDropMove((PieceType)drop_piece_type, (Square)to_square).value();
}

int __Board::move_from_usi(const std::string& usi) const {
  return usiToMove(pos, usi).value();
}

int __Board::move_from_csa(const std::string& csa) const {
  return csaToMove(pos, csa).value();
}

int __Board::move_from_move16(const unsigned short move16) const {
  return move16toMove(Move(move16), pos).value();
}

int __Board::move_from_psv(const unsigned short move16) const {
  return move16toMove(Move(__move16_from_psv(move16)), pos).value();
}

int __Board::turn() const { return pos.turn(); }
void __Board::setTurn(const int turn) { pos.setTurn((Color)turn); }
int __Board::ply() const { return pos.gamePly(); }
void __Board::setPly(const int ply) { pos.setStartPosPly(ply); }
std::string __Board::toSFEN() const { return pos.toSFEN(); }
std::string __Board::toCSAPos() const { return pos.toCSAPos(); }
void __Board::toHuffmanCodedPos(char* data) const { pos.toHuffmanCodedPos((u8*)data); }
void __Board::toPackedSfen(char* data) const { pos.toPackedSfen((u8*)data); }
int __Board::piece(const int sq) const { return (int)pos.piece((Square)sq); }
int __Board::kingSquare(const int c) { return (int)pos.kingSquare((Color)c); }
bool __Board::inCheck() const { return pos.inCheck(); }
int __Board::mateMoveIn1Ply() { return pos.mateMoveIn1Ply().value(); }

int __Board::mateMove(int ply) {
  if (pos.inCheck())
...
bool __Board::is_mate(int ply) {
  return mateMoveInEvenPly(pos, ply) > 0;
}

unsigned long long __Board::getKey() const { return pos.getKey(); }
bool __Board::moveIsPseudoLegal(const int move) const { return pos.moveIsPseudoLegal<false>(Move(move)); }

bool __Board::pseudoLegalMoveIsLegal(const int move) const {
  const Bitboard pinned = pos.pinnedBB();
  return pos.pseudoLegalMoveIsLegal<false, false>(Move(move), pinned);
}

bool __Board::moveIsLegal(const int move) const {
  return moveIsPseudoLegal(move) && pseudoLegalMoveIsLegal(move);
}

bool __Board::is_nyugyoku() const { return nyugyoku(pos); }
bool __Board::isOK() const { return pos.isOK(); }

std::vector<int> __Board::pieces_in_hand(const int color) const {
...
std::vector<int> __Board::pieces() const {
...
void __Board::piece_planes(char* mem) const {
  // P1 piece 14 planes
  // P2 piece 14 planes
...
// 白の場合、盤を反転するバージョン
void __Board::piece_planes_rotate(char* mem) const {
  // P1 piece 14 planes
  // P2 piece 14 planes
  if (pos.turn() == Black) {
    // 黒の場合
...
void __Board::_dlshogi_make_input_features(char* mem1, char* mem2) const {
...
unsigned long long __Board::bookKey() {
  return Book::bookKey(pos);
}

unsigned long long __Board::bookKeyAfter(const unsigned long long key, const int move) {
  return Book::bookKeyAfter(pos, key, Move(move));
}

void __Board::bbToVector(PieceType pt, Color c, Piece piece, std::vector<int>& board) const {
  Bitboard bb = pos.bbOf(pt, c);
  while (bb) {
    const Square sq = bb.firstOneFromSQ11();
    board[sq] = piece;
...
__LegalMoveList::__LegalMoveList() {}
__LegalMoveList::__LegalMoveList(const __Board& board) {
  ml.reset(new MoveList<LegalAll>(board.pos));
}

bool __LegalMoveList::end() const { return ml->end(); }
int __LegalMoveList::move() const { return ml->move().value(); }
void __LegalMoveList::next() { ++(*ml); }
int __LegalMoveList::size() const { return (int)ml->size(); }

__PseudoLegalMoveList::__PseudoLegalMoveList() {}
__PseudoLegalMoveList::__PseudoLegalMoveList(const __Board& board) {
  ml.reset(new MoveList<PseudoLegal>(board.pos));
}

bool __PseudoLegalMoveList::end() const { return ml->end(); }
int __PseudoLegalMoveList::move() const { return ml->move().value(); }
void __PseudoLegalMoveList::next() { ++(*ml); }
int __PseudoLegalMoveList::size() const { return (int)ml->size(); }
...
__DfPn::__DfPn() {}
__DfPn::__DfPn(const int max_depth, const uint32_t max_search_node, const int draw_ply)
    : dfpn(max_depth, max_search_node, draw_ply) {}

bool __DfPn::search(__Board& board) {
  pv.clear();
  return dfpn.dfpn(board.pos);
}

bool __DfPn::search_andnode(__Board& board) {
  return dfpn.dfpn_andnode(board.pos);
}

void __DfPn::stop(bool is_stop) {
  dfpn.dfpn_stop(is_stop);
}

int __DfPn::get_move(__Board& board) {
  return dfpn.dfpn_move(board.pos).value();
}

void __DfPn::get_pv(__Board& board) {
  pv.clear();
  dfpn.get_pv(board.pos, pv);
}

void __DfPn::set_draw_ply(const int draw_ply) {
  dfpn.set_draw_ply(draw_ply);
}

void __DfPn::set_maxdepth(const int depth) {
  dfpn.set_maxdepth(depth);
}

void __DfPn::set_max_search_node(const uint32_t max_search_node) {
  dfpn.set_max_search_node(max_search_node);
}

uint32_t __DfPn::get_searched_node() {
